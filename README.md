...
基于 LangGraph 的只读文件版本治理 Agent。当前版本 `0.5.2` 在 `0.5.1` 应用
数据库骨架上新增 LangChain 多模型 Profile：Content、Version、Evidence 可分别
路由 Claude、Gemini、GLM、DeepSeek、Qwen、OpenAI 及 LangChain 其他主流
Provider，也可使用 OpenRouter、LiteLLM 和标准 OpenAI 兼容中转站。旧单模型请求和
checkpoint 自动转换为默认 Profile，同时继续通过兼容性、确定性回退、checkpoint
泄漏和数据库迁移测试保护治理结论。当前具备：
...
只建立应用数据库、Repository 和迁移；`0.5.2` 只在三个既有 Subagent 子图内增加
`resolve_model_profile` 节点，不改变确定性事实计算和顶层 LangGraph 顺序。
...
- 独立应用数据库路径、父目录自动创建、SQL 日志和文件锁等待配置。

当前 CLI 以请求 JSON 为直接运行配置；YAML 用于记录统一部署默认值。0.5.2 的
应用数据库配置尚未接入 CLI 和主图，迁移命令直接读取 `alembic.ini`。
...
```bash
docker build --build-arg APP_VERSION=0.5.2 -t file-manage-agent:0.5.2 .
```
...
```bash
docker build \
  --build-arg APP_VERSION=0.5.2 \
  --build-arg LLM_EXTRAS=anthropic,deepseek,qwen \
  -t file-manage-agent:0.5.2-mainstream .
```

默认显示 CLI 帮助：

```bash
docker run --rm file-manage-agent:0.5.2
```
...
- 主图持久化工具调用审计；0.5.2 仍只建立审计表和 Repository；
- 未安装的可选 LangChain Provider 包；基础安装不会一次性包含全部模型 SDK；
- 主图长期 Memory、Context Compact、Skills、邮件 MCP 证据和 Worktree；
