...
基于 LangGraph 的只读文件版本治理 Agent。当前版本 `0.5.3` 在 `0.5.2`
LangChain 多模型适配上新增 Task 级 Skills：顶层只加载注册表元数据，Team
Orchestration 只为当前分派读取并绑定所需 `SKILL.md`，Subagent 完成后立即清空
指令正文并恢复 `available`。Content、Version、Evidence 仍可分别路由 Claude、
Gemini、GLM、DeepSeek、Qwen、OpenAI 及其他主流 Provider 和第三方中转站，同时
继续通过兼容性、确定性回退、checkpoint 泄漏和数据库迁移测试保护治理结论。
当前具备：
...
元数据节点，并在 Team Orchestration 分派边界内完成按需加载和释放，不改变四个
确定性业务子图的事实计算。
...
```bash
docker build --build-arg APP_VERSION=0.5.3 -t file-manage-agent:0.5.3 .
```
...
```bash
docker build \
  --build-arg APP_VERSION=0.5.3 \
  --build-arg LLM_EXTRAS=anthropic,deepseek,qwen \
  -t file-manage-agent:0.5.3-mainstream .
```

默认显示 CLI 帮助：

```bash
docker run --rm file-manage-agent:0.5.3
```
...
- 主图持久化工具调用审计；0.5.3 仍只建立审计表和 Repository；
- 未安装的可选 LangChain Provider 包；基础安装不会一次性包含全部模型 SDK；
- 主图长期 Memory、Context Compact、邮件 MCP 证据和 Worktree；
