...
基于 LangGraph 的只读文件版本治理 Agent。当前版本 `0.5.1` 在 `0.5.0` 固定
Agent Team 上新增独立应用数据库骨架：Inventory 后分派 Content、Version Analysis
内部调用 Version、Evidence 后分派 Evidence，并通过兼容性、回退、checkpoint
泄漏和数据库迁移测试保护确定性治理结论。当前具备：
...
只建立应用数据库、Repository 和迁移，不改变顶层 LangGraph 的执行顺序。
...
state = create_initial_state(
    ...
    # 0.5.1 仍默认关闭真实模型；阶段分派会使用离线 Mock Provider。
...
当前 CLI 以请求 JSON 为直接运行配置；YAML 用于记录统一部署默认值。0.5.1 的
...
```bash
docker build --build-arg APP_VERSION=0.5.1 -t file-manage-agent:0.5.1 .
```

默认显示 CLI 帮助：

```bash
docker run --rm file-manage-agent:0.5.1
```
...
- 主图持久化工具调用审计；0.5.1 只建立审计表和 Repository；
