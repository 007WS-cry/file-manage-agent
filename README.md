...
基于 LangGraph 的只读文件版本治理 Agent。当前版本 `0.4.4` 已把三个固定 Subagent
接入真实业务阶段：Inventory 后分派 Content、Version Analysis 内部调用 Version、
Evidence 后分派 Evidence，并在模型或协议失败时保留确定性治理结论。当前具备：
...
`0.4.4` 已由顶层图和 Version Analysis 子图构造最小 `dispatch_request`。模型只解释
...
state = create_initial_state(
    ...
    # 0.4.4 默认值仍为关闭；阶段分派会使用离线 Mock Provider。
...
```bash
docker build --build-arg APP_VERSION=0.4.4 -t file-manage-agent:0.4.4 .
```

默认显示 CLI 帮助：

```bash
docker run --rm file-manage-agent:0.4.4
```
