...
def test_release_version_is_consistent_across_package_and_docker() -> None:
    """Python 包、项目元数据、Docker 默认值和 README 应统一为 0.6.4。"""
    pyproject = (PROJECT_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    dockerfile = (PROJECT_ROOT / "Dockerfile").read_text(encoding="utf-8")
    readme = (PROJECT_ROOT / "README.md").read_text(encoding="utf-8")

    assert app.__version__ == "0.6.4"
    assert 'version = "0.6.4"' in pyproject
    assert "ARG APP_VERSION=0.6.4" in dockerfile
    assert "当前版本 `0.6.4`" in readme
