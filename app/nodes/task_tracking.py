...
from app.state.models import ErrorRecord, FileGovernanceState, TaskItem, TaskStatusUpdate
from app.utils.runtime import utc_now_iso

"""本模块把顶层文件治理阶段确定性映射为 Task 状态，并始终通过编排子图刷新 Todo。"""
...
def _has_orchestration_failure(errors: list[ErrorRecord]) -> bool:
    ...
    *,
    ...
    if current is not None:
        ...
        if status == "completed" and current_status == "pending":
            running_state, running_errors = _apply_task_status(
                state,
                task_type,
                "running",
            )
            if _has_orchestration_failure(running_errors):
                return running_state, running_errors
            completed_state, completed_errors = _apply_task_status(
                running_state,
                task_type,
                "completed",
...
def _block_downstream_tasks(
    ...
    for task_type in TASK_EXECUTION_ORDER[failed_index + 1 : -1]:
        working_state, update_errors = _apply_task_status(
            working_state,
            task_type,
            "skipped",
        ...
        if _has_orchestration_failure(update_errors):
            break
    return working_state, new_errors


def _sync_business_task_status(
    state: FileGovernanceState,
    task_type: str,
    *,
    ...
    if fatal_errors:
        ...
        new_errors.extend(update_errors)
        if not _has_orchestration_failure(update_errors):
        ...
        return _public_task_update(working_state, new_errors)
    ...
    if task_type == "recommendation":
        ...
        working_state, human_errors = _apply_task_status(
            working_state,
            "human_review",
            human_status,
        )
        ...
        working_state, next_errors = _apply_task_status(
            working_state,
            next_task_type,
            "running",
        )
...
def _settle_unfinished_tasks_before_report(
    state: FileGovernanceState,
) -> tuple[FileGovernanceState, list[ErrorRecord]]:
...
def plan_run_tasks(state: FileGovernanceState) -> dict:
    ...
    """
    working_state, errors = _apply_task_status(state, "inventory", "running")
    return _public_task_update(working_state, errors)
...
def sync_inventory_task_status(state: FileGovernanceState) -> dict:
    ...
    """
    return _sync_business_task_status(
        state,
        "inventory",
...
def sync_human_review_task_status(state: FileGovernanceState) -> dict:
    ...
    """
    working_state, errors = _apply_task_status(
        state,
        "human_review",
        "completed",
...
def sync_report_task_status(state: FileGovernanceState) -> dict:
    ...
    """
    working_state, errors = _settle_unfinished_tasks_before_report(state)
    if _has_orchestration_failure(errors):
        return _public_task_update(working_state, errors)
    working_state, running_errors = _apply_task_status(
        working_state,
        "report",
        "running",
    )
    ...
    if _has_orchestration_failure(running_errors):
        return _public_task_update(working_state, errors)
    working_state, completed_errors = _apply_task_status(
        working_state,
        "report",
        "completed",
        output_refs=("report",),
