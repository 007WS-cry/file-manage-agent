...
后台恢复请求必须同时匹配当前 `run_id`、`interrupt_id` 和 `kind`。`request_id`
是幂等键，不是授权凭据；API 网关仍应对提交、状态、恢复和报告接口执行身份认证、
租户隔离、速率限制和审计。调用方不得使用新的幂等键反复尝试绕过已拒绝的旧中断。

应用数据库与 checkpoint 必须持久化在受访问控制的卷中。等待人工输入期间可能跨 API
或 Worker 重启，因此不能手工编辑数据库 JSON 列、删除 checkpoint 或重置
`attempt_count`/`resume_count`。恢复型自由文本说明不得进入长期 Memory、报告或公开
日志。

报告下载不能信任数据库登记路径。服务端会根据原任务的 `report_root` 重新验证普通
文件、`.md` 扩展名、符号链接、路径边界和大小。反向代理不应绕过该接口直接暴露整个
报告或产物目录。

## 1.0.0 PostgreSQL 与备份恢复

PostgreSQL 演示仅使用 Docker Compose。密码必须保存在未提交的 `.env`、容器 Secret
或等效秘密管理系统中，不得写入 Compose 文件、请求 JSON、状态、checkpoint、命令
历史或测试快照。示例 URL 中的密码必须替换为虚构值；Compose 字符串插值不负责 URL
编码，应优先使用 URL 非保留字符或独立 Secret 注入。

`scripts/backup_restore_demo.py` 的 SQLite `roundtrip` 只恢复到新副本；PostgreSQL
`roundtrip` 只创建带 `file_governance_restore_` 前缀的临时 Docker 数据库并在验证后
删除。显式 `restore` 必须提供确认参数，且仍只允许创建新目标。生产恢复必须：

- 停止或隔离写入；
- 校验备份来源、权限、完整性和加密；
- 恢复到新文件或新数据库；
- 核对 Alembic 版本、表数量、关键行数和应用健康；
- 使用可回滚的连接切换，不直接覆盖活动数据库；
- 在确认回滚窗口结束后再按保留策略清理旧数据。

删除 Compose named volume 是不可恢复操作。普通停止服务不得附加 `--volumes`。

## 1.0.0 演示与输入不变性

演示数据生成器拒绝覆盖非空目录，所有输出应位于 `.artifacts` 等受控目录。真实业务
输入不得作为演示生成目标。E2E 运行前后会比较普通文件数量、相对路径、大小和
SHA-256；任一差异均应视为发布阻断问题，而不是自动“修复”输入。

`max_files` 和 LangGraph `max_concurrency` 只提供应用层上限，不能替代容器 CPU、内存、
进程数、超时、只读挂载和网络策略。100–500 文件容量演示应使用隔离的脱敏数据集，
并预先估算版本分组的文件对比较成本。

## 1.0.2 语义变更与审核规则

Version Subagent 只能接收有数量和字符上限的关键字段、段落与结构差异，不接收完整
文档正文。每项模型语义分类必须引用当前比较输入已有的 `diff:` 证据，且 old/new 值
必须能在所引证据中逐字复核。伪造引用、跨 comparison 引用、非法类型、重复引用或
无法复核的新旧值都会拒绝整项模型输出，并使用确定性回退。

LLM 不得返回自动批准、删除、发送、移动等系统动作，也不得直接设置审核优先级。
人工审核优先级只由固定规则根据已校验类型和重要性计算；纯措辞、格式及无实质变化
不会因为模型给出高重要性而升级为高优先级。语义分析只能影响审核顺序和是否强制
人工确认，不能修改文件、版本方向、相似度、证据关系或主版本候选评分。

## 1.0.3 双轨版本关系安全边界

Version Subagent 只能基于有数量和字符上限的 `relation_evidence` 提出封闭关系候选，
不能返回建边、删除、移动、发送或批准等系统动作。非 `uncertain` 候选必须引用输入
白名单中的 `diff:` 证据；伪造、重复或 comparison 不匹配的引用会拒绝模型输出。

原始 SHA-256 完全一致属于不可覆盖事实。直接修订、导出、语义重复、并行分支和无关
文件候选必须分别满足确定性约束；时间、版本名、内容和结构不支持父子关系时，LLM
不能建立有向边。两条轨道冲突或候选违反硬约束时，融合结果必须为不确定并进入人工
审核。报告和中断只展示受控关系、理由、置信度与证据引用，不加入完整文档正文。
